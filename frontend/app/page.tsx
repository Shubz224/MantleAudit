...
export default function MantlePage() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#0B0E11] overflow-x-hidden font-sans text-white selection:bg-[#5ac2b5] selection:text-black">
      {/* Animated Background - Darker with Glass Effect */}
      <div className={`mantle-bg-gradient fixed top-0 left-0 w-full h-full z-0 transition-all duration-700 ${scrolled ? 'blur-lg opacity-20' : 'opacity-30'}`}></div>
      <div className={`mantle-bg-overlay fixed top-0 left-0 w-full h-full z-[1] pointer-events-none transition-all duration-700 ${scrolled ? 'opacity-20' : 'opacity-15'}`}></div>

      {/* Scroll Vignette / Darkener */}
      <div className={`fixed inset-0 z-[1] bg-[#0B0E11]/60 backdrop-blur-sm transition-opacity duration-700 pointer-events-none ${scrolled ? 'opacity-60' : 'opacity-0'}`}></div>

      {/* Floating particles */}
      <div className={`particles fixed top-0 left-0 w-full h-full z-[2] pointer-events-none transition-all duration-700 ${scrolled ? 'blur-sm opacity-30' : 'opacity-60'}`}>
        {[...Array(20)].map((_, i) => (
      ...
      <div className={`mantle-logo-container fixed z-50 transition-all duration-[1500ms] ease-in-out mix-blend-difference
        ...
        ${!intro && scrolled ? 'blur-sm scale-90 opacity-40' : ''}
      ...
      <div className={`relative z-10 min-h-screen flex items-center justify-center p-8 pt-32 md:pt-32 transition-opacity duration-1000 delay-500 ${intro ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'}`}>
        <div className="w-full max-w-[1200px] flex flex-col items-center gap-12 max-md:gap-8">
          ...
          <div className={`relative text-center group transition-transform duration-700 ease-out ${!scrolled ? 'translate-x-[8vw]' : 'translate-x-0'}`}>
          ...
          <a href="/dashboard" className="opacity-0 animate-[fadeInUp_1s_ease-out_0.5s_forwards]">
            <button className="group relative backdrop-blur-xl bg-[#0B0E11]/60 border border-[#6ED6C9] text-[#6ED6C9] font-['Orbitron',_sans-serif] text-lg font-bold py-5 px-14 cursor-pointer flex items-center gap-4 uppercase tracking-[0.15em] transition-all duration-300 overflow-hidden hover:bg-[#6ED6C9]/10 hover:shadow-[0_0_30px_rgba(110,214,201,0.2)] rounded-xl">
              <span className="absolute top-0 left-0 w-[2px] h-full bg-[#6ED6C9] animate-[heightGlitch_2s_infinite]"></span>
              <span className="absolute top-0 right-0 w-[2px] h-full bg-[#6ED6C9] animate-[heightGlitch_2s_infinite_reverse]"></span>

              <span className="relative z-10 group-hover:text-white transition-colors duration-300">Enter the Vault</span>
              <span className="relative z-10 text-xl transition-transform duration-300 group-hover:translate-x-2 group-hover:text-white">→</span>

              {/* Button bg fill effect */}
              <span className="absolute inset-0 bg-[#6ED6C9] transform scale-x-0 origin-left transition-transform duration-300 group-hover:scale-x-100 opacity-20"></span>
            </button>
          </a>

          {/* Description - Liquid Glass Card */}
          <div className="relative backdrop-blur-xl bg-[#0B0E11]/80 border border-white/[0.06] p-10 max-w-[900px] shadow-2xl rounded-xl opacity-0 animate-[fadeInUp_1s_ease-out_0.6s_forwards] hover:border-[#6ED6C9]/30 hover:shadow-[0_0_80px_rgba(110,214,201,0.1)] transition-all duration-500 group overflow-hidden">
            {/* Tech scanline */}
            <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#6ED6C9] to-transparent -translate-x-full group-hover:animate-[scanline_2s_linear_infinite]"></div>

            <p className="text-[clamp(1rem,2vw,1.25rem)] leading-relaxed text-[#E6EDF3] text-center m-0 relative z-10">
              The first <span className="text-[#6ED6C9] font-semibold drop-shadow-[0_0_8px_rgba(110,214,201,0.3)]">Zero-Knowledge Compliance Bridge</span> that solves the "Transparency Paradox" in institutional DeFi.
              We enable hedge funds and institutions to execute <span className="text-[#6ED6C9] font-semibold drop-shadow-[0_0_8px_rgba(110,214,201,0.3)]">Dark Pool-style strategies</span> on public liquidity
              without leaking alpha, while remaining fully compliant with global AML regulations.
            </p>
            {/* Corner Markers */}
            <div className="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-[#6ED6C9]/30"></div>
            <div className="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-[#6ED6C9]/30"></div>
          </div>
          ...
          <div className="relative backdrop-blur-xl bg-[#0B0E11]/80 border-y border-white/[0.06] rounded-xl p-12 w-full shadow-2xl overflow-hidden opacity-0 animate-[fadeInUp_1s_ease-out_1.6s_forwards] hover:border-[#6ED6C9]/20 transition-all duration-500">
            <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#6ED6C9] to-transparent animate-[shimmer_3s_ease-in-out_infinite] opacity-30"></div>
            <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#6ED6C9] to-transparent animate-[shimmer_3s_ease-in-out_infinite] opacity-30 animation-delay-1500"></div>
            <blockquote className="font-sans text-[clamp(1.1rem,2.5vw,1.5rem)] leading-relaxed text-[#E6EDF3] font-light italic text-center tracking-wide">
              "Privacy is not about hiding crimes; it's about <span className="text-white font-normal underline decoration-[#6ED6C9]/50 underline-offset-4 decoration-1">protecting value</span>.
